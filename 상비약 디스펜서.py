...
def confirm_settings(user):
    ...
    yes_btn.pack()
    no_btn = Button(win, text="아니오", font=("함초롬돋움", 15), command=lambda: show_age_buttons(user))
    no_btn.pack()
...
def prescription_setting_suceed():
    ...
    home_btn.place(relx=0.4, rely=0.9)
...
def reset_prescription():
    ...
    yes_btn.pack()
    no_btn = Button(win, text="아니오", font=("함초롬돋움", 15), command=go_back)
    no_btn.pack()
...
# 사용자 설정 중 뒤로 가기 버튼과 초기 홈 버튼을 띄우는 함수
def pre_add_nav_buttons():
    # 뒤로 가기 버튼
    back_btn = Button(win, text="뒤로 가기", font=("함초롬돋움", 15), command=go_back)
    back_btn.place(relx=0.6, rely=0.9)  # 아래쪽에 배치
    # 처음 화면으로 버튼
    home_btn = Button(win, text="처음 화면으로", font=("함초롬돋움", 15), command=pre_go_home)
...
def go_back(label=None):
    ...
    if history:
        ...
        if history:
            ...
            elif last_screen == "set_prescription":
                set_prescription()
            elif last_screen == "pre_suceed":
                prescription_setting_suceed()
            elif last_screen == "reset_prescription":
                reset_prescription()
            elif last_screen == "check_barcode":
                check_barcode()
            elif last_screen == "barcode_scan":
                barcode_scan()
